...
def read_out(filename):
    ...
    if(err==1):
        ...
        return subcatchResultValueList,nodeResultValueList,linkResultValueList,sub_name,node_name,link_name


def save_data(node,file):
    T=len(node)
    N=len(node[0])
    M=len(node[0][0])
    ...
    for iten in range(M):
        ...
        for t in range(T):
            for n in range(N):
                text+=str(node[t][n][iten][0])+'\t\t'
            text+='\n'
        
        output = open('./'+file+'/data'+str(iten)+'.txt', 'wt')
        output.write(text)
        output.close()
...
if __name__=='__main__':
    ...
    filename='./otbc.out'
    sub,node,link,sub_name,node_name,link_name=read_out(filename)
    ...
    save_data(node,'node_output')
    save_data(link,'link_output')
    save_data(sub,'sub_output')
    
    '''
    filename='./ot.inptem_rain.out'
    data,name=read_out(filename)
